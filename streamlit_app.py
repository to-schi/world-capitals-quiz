    ...
    if country != st.session_state['last'] and country != "":
        ...
        if st.session_state['country'] == world[capital]:
            ...
            if st.session_state['number'] == -1:
                ...
                second.write(f"You know all capitals in the world (ɔ °0°)ɔ and answered {st.session_state['right']} of {st.session_state['questions']} questions correctly ({int(round(perc,0))} percent).")
                second.write(f"That's {grade(perc)}")
                reset()
            else:
                second.write("That's correct. (✿^‿^)")
            ...
            second.write(f"'{country}' is wrong. (ɔ •︵•)ɔ  **{capital}** is the capital of: **{world[st.session_state['capital']]}**")
            second.write(f"{st.session_state['right']} of {st.session_state['questions']} ({str(st.session_state['number']+1)} countries remaining)")
